...
import filterUsersPhoneByInput from "@/shared/lib/utils/filterUsersPhoneByInput";
import { deleteUserPhone, updateUserPhone } from "@/shared/api/requests";
...
const TableUsers: FC<{ userPhones: UserPhone[], onlyRead?: boolean }> = ({ userPhones, onlyRead = false }) => {
    ...
    const { setSteckMessages } = usePopup();
        ...
        try {
            ...
            if (dataToSend) {
                ...
                if (response.code >= 200 && response.code <= 299) {
                    ...
                    setSteckMessages((prevMessages) => [{ isErrorPopup: true, message: 'Сессия закончилась, пройдите аутентификацию!' }, ...prevMessages]);      
                }
            }
        } catch (error) {
    ...
    const deleteUserPhoneInTable = async (id: number) => {
        try {
            ...
            if (response.code >= 200 && response.code <= 299) {
                setSteckMessages((prevMessages) => [{ isErrorPopup: false, message: 'Пользователь успешно удален из базы данных. Сайт скоро обновится, чтобы данные синхронизировались!' }, ...prevMessages]);
                setTimeout(() => { location.reload(); }, 1000);
            } else if (response.code === 401) {
                setSteckMessages((prevMessages) => [{ isErrorPopup: true, message: 'Сессия не может быть открыта для вам. Вам следует войти в панель администратора! Сайт переносит вас на страницу входа в панель администрации.' }, ...prevMessages]);
                setTimeout(() => { setIsAuthenticated(false); }, 3000);
            } else {
                setSteckMessages((prevMessages) => [{ isErrorPopup: true, message: `Произошла ошибка при удалении ${id} пользователя` }, ...prevMessages]);
            }
        } catch (error) {
            setSteckMessages((prevMessages) => [{ isErrorPopup: true, message: 'Произошла ошибка во время работы сервера' }, ...prevMessages]);
        }

    }

    let resultFilter = useMemo(() => filterUsersPhoneByInput(updatedUserPhones, searchInput), [searchInput, updatedUserPhones]);
                                        ...
                                        <tr key={userPhone.user_phone_id} className={`border-t border-accent h-[75px] ${number % 2 === 0 ? 'bg-surface' : ''}`} >
                                            ...
                                            <td className="px-5 text-right">
                                                {onlyRead ?
                                                    ''
                                                    :
                                                    <div className="cursor-pointer hover:opacity-40 duration-500 ease-in-out" onClick={() => { deleteUserPhoneInTable(userPhone.user_phone_id) }}><img src="/assets/svg/trash.svg" alt="trash_icon" className="inline-block" /></div>
                                                }
                                            </td>
                                        </tr>
                                    )
